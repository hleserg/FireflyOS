...
| H5 | Which wake sources are usable in practice, and what does each cost? **The Waveshare's QMI8658 INT1 is now known to reach a wake-capable GPIO** — GPIO21, inside the ESP32-S3's RTC-IO range 0–21 — so a raise gesture is an interrupt wake candidate, not necessarily a poll, on both boards ([WAVESHARE_ARRIVAL §3.2a](WAVESHARE_ARRIVAL.md), OD-16) | UNKNOWN — routing VERIFIED, cost still unmeasured | measurement; vendor table gives the shape |
...
| H16 | **The QMI8658C's INT1 electrical configuration** — push-pull or open-drain, active level. The schematic gives the net (`QMI_INT1` → GPIO 21, [WAVESHARE_ARRIVAL §3.2a](WAVESHARE_ARRIVAL.md)) but draws no pull resistor, and the setting is a `CTRL1` register field, not a schematic fact | UNKNOWN | QMI8658C datasheet `CTRL1`, then confirm against the strap/board on a bench before firmware relies on a polarity |
