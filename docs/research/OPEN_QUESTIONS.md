...
| ~~A9~~ | ~~Does the day theme keep its near-white page on the AMOLED board?~~ | **RESOLVED — no, option 4** | the project owner, on [issue #52](https://github.com/hleserg/Attadipa/issues/52), 2026-08-22: automatic by panel technology, and the theme toggle is unchanged — the user still picks day or night, and day renders against a dark page on an emissive panel. Recorded as [OWNER_DECISIONS.md](OWNER_DECISIONS.md) OD-16. The plumbing (`ui::PixelCost`, mapped from `platform::PanelTechnology` by the composition root, and the emissive-day palette in `ui/src/color.cpp`) landed the same day |
