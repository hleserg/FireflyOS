...
| Main I2C bus | — | SDA 15, SCL 14 — **and both are brought out on the expansion pad row below**, labelled there as bare `IO15`/`IO14` | **five confirmed**: `0x18`, `0x34`, `0x38`, `0x40`, `0x51`. The sixth is the IMU, and it is `0x6A` on one datasheet revision and `0x6B` on another (row above, `address CONFLICTING`) — **whether `0x6A` is free or already the IMU's own address is `UNKNOWN`, not a fact**, until a bus scan settles which revision is fitted. Folded into T-106 rather than left as a separate claim | — | VERIFIED |
